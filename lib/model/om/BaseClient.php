...
abstract class BaseClient extends BaseObject  implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->phone = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = ClientPeer::NUM_COLUMNS - ClientPeer::NUM_LAZY_LOAD_COLUMNS).
	 ...
	 * related Orders from storage. If this Client is new, it will return
		...
		if ($this->collOrders === null) {
			if ($this->isNew()) {
			   $this->collOrders = array();
				...
				$criteria->add(OrderPeer::CLIENT_ID, $this->id);
	 ...
	 */
	public function getOrdersJoinElement($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ClientPeer::DATABASE_NAME);
		}
		...
		if ($this->collOrders === null) {
			if ($this->isNew()) {
				...
				$this->collOrders = OrderPeer::doSelectJoinElement($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
