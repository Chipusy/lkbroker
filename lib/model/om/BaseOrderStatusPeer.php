...
abstract class BaseOrderStatusPeer {
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseOrderStatusPeer', $criteria, $con);
		}
	...
	{
		return $withPrefix ? OrderStatusPeer::CLASS_DEFAULT : OrderStatusPeer::OM_CLASS;
	}
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseOrderStatusPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseOrderStatusPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseOrderStatusPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseOrderStatusPeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseOrderStatusPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseOrderStatusPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseOrderStatusPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseOrderStatusPeer', $values, $con, $ret);
    }
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += OrderStatusPeer::doOnDeleteCascade(new Criteria(OrderStatusPeer::DATABASE_NAME), $con);
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseOrderStatusPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
