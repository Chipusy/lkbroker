...
abstract class BaseCategory extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array CategoryPreference[] Collection to store aggregation of CategoryPreference objects.
	 */
	protected $collCategoryPreferences;

	/**
	 * @var        Criteria The criteria used to select the current contents of collCategoryPreferences.
	 */
	private $lastCategoryPreferenceCriteria = null;

	/**
	 * @var        array Element[] Collection to store aggregation of Element objects.
	 */
	protected $collElements;

	/**
	 * @var        Criteria The criteria used to select the current contents of collElements.
	 */
	private $lastElementCriteria = null;
	...
	public function setHeader($v)
	{
		...
		if ($this->header !== $v) {
			$this->header = $v;
			$this->modifiedColumns[] = CategoryPeer::HEADER;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->header = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collCategoryPreferences = null;
			$this->lastCategoryPreferenceCriteria = null;

			$this->collElements = null;
			$this->lastElementCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collCategoryPreferences !== null) {
				foreach ($this->collCategoryPreferences as $referrerFK) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setHeader($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CategoryPeer::HEADER)) $criteria->add(CategoryPeer::HEADER, $this->header);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getCategoryPreferences() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addCategoryPreference($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collCategoryPreferences collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addCategoryPreferences()
	 */
	public function clearCategoryPreferences()
	{
		$this->collCategoryPreferences = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collCategoryPreferences collection (array).
	 *
	 * By default this just sets the collCategoryPreferences collection to an empty array (like clearcollCategoryPreferences());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initCategoryPreferences()
	{
		$this->collCategoryPreferences = array();
	}

	/**
	 * Gets an array of CategoryPreference objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Category has previously been saved, it will retrieve
	 * related CategoryPreferences from storage. If this Category is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array CategoryPreference[]
	 * @throws     PropelException
	 */
	public function getCategoryPreferences($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CategoryPeer::DATABASE_NAME);
		}
		...
		if ($this->collCategoryPreferences === null) {
			if ($this->isNew()) {
			   $this->collCategoryPreferences = array();
				...
				$criteria->add(CategoryPreferencePeer::CATEGORY_ID, $this->id);

				CategoryPreferencePeer::addSelectColumns($criteria);
				$this->collCategoryPreferences = CategoryPreferencePeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				CategoryPreferencePeer::addSelectColumns($criteria);
				if (!isset($this->lastCategoryPreferenceCriteria) || !$this->lastCategoryPreferenceCriteria->equals($criteria)) {
					$this->collCategoryPreferences = CategoryPreferencePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastCategoryPreferenceCriteria = $criteria;
		return $this->collCategoryPreferences;
	}

	/**
	 * Returns the number of related CategoryPreference objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related CategoryPreference objects.
	 * @throws     PropelException
	 */
	public function countCategoryPreferences(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CategoryPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collCategoryPreferences === null) {
			if ($this->isNew()) {
				...
				$count = CategoryPreferencePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastCategoryPreferenceCriteria) || !$this->lastCategoryPreferenceCriteria->equals($criteria)) {
					...
					$count = count($this->collCategoryPreferences);
				}
			} else {
				$count = count($this->collCategoryPreferences);
			}
	...
	/**
	 * Method called to associate a CategoryPreference object to this object
	 * through the CategoryPreference foreign key attribute.
	 *
	 * @param      CategoryPreference $l CategoryPreference
	 * @return     void
	 * @throws     PropelException
	 */
	public function addCategoryPreference(CategoryPreference $l)
	{
		if ($this->collCategoryPreferences === null) {
			$this->initCategoryPreferences();
		}
		if (!in_array($l, $this->collCategoryPreferences, true)) { // only add it if the **same** object is not already associated
			array_push($this->collCategoryPreferences, $l);
			$l->setCategory($this);
		}
	}

	/**
	 * Clears out the collElements collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	...
	/**
	 * Initializes the collElements collection (array).
	 *
	...
	public function addElement(Element $l)
	{
		...
		if (!in_array($l, $this->collElements, true)) { // only add it if the **same** object is not already associated
			array_push($this->collElements, $l);
			$l->setCategory($this);
		}
	}
	 ...
	 */
	public function getElementsJoinElementStatus($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CategoryPeer::DATABASE_NAME);
		}
		...
		if ($this->collElements === null) {
			if ($this->isNew()) {
				...
				$this->collElements = ElementPeer::doSelectJoinElementStatus($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	{
		if ($deep) {
			if ($this->collCategoryPreferences) {
				foreach ((array) $this->collCategoryPreferences as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collElements) {
				foreach ((array) $this->collElements as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
